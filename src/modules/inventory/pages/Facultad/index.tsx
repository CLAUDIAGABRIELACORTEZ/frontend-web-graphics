...
const UserPage = (): JSX.Element => {
  ...
  const { allResource: allFacultades, countData, isLoading, mutate } = useGetAllResource('http://localhost:3000/api/estadisticas-academicas/filter?periodo=2020-1&localidad=SANTA CRUZ&facultad=Todas&modalidad=PRESENCIAL')
  const { deleteResource: deleteUser } = useDeleteResource(ENDPOINTS.USER)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [, setSearchProduct] = useState('')
  console.log(allFacultades)
      ...
      <Card x-chunk="dashboard-06-chunk-0" className='flex flex-col overflow-hidden w-full relative'>
          ...
          <div className='overflow-x-auto'>
            <Table>
              ...
              <TableBody>
                {isLoading
                  ? <div>Cargando</div>
                  : allFacultades?.map((facultad: Facultad) => (
                      <TableRow key={facultad.id}>
                        <TableCell>{facultad.nombre_facultad}</TableCell>
                        <TableCell>{facultad.retirados}</TableCell>
                        <TableCell>{facultad.egresados}</TableCell>
                        {/* <TableCell>{facultad.periodo ? facultad.branch.name : '-'}</TableCell> */}
                        {/* <TableCell>
                          <Badge variant={!facultad.is_active ? 'default' : 'outline'}>
                            {!facultad.is_active ? 'Activo' : 'Inactivo'}
                          </Badge>
                        </TableCell> */}
                        <TableCell>
                          <DropdownMenu onOpenChange={() => { setIsDialogOpen(false) }}>
                            ...
                            <DropdownMenuContent align="end" className='bg-light-bg-primary'>
                              ...
                              <DropdownMenuItem onClick={() => { navigate(`${PrivateRoutes.USER}/${facultad.id}`) }}>
                                ...
                                <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                                  ...
                                  <AlertDialogContent className='bg-light-bg-primary'>
                                    ...
                                    <AlertDialogFooter>
                                      ...
                                      <AlertDialogAction onClick={() => { deletePermanentlyUser(facultad.id) }}>Continue</AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
                                </AlertDialog>
                              </DropdownMenuItem>
        ...
        <CardFooter className='w-full'>
          {/* <Pagination
            ...
            // currentItems={allFacultad?.length ?? 0}
